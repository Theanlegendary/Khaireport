...
def generate_reports_from_data(export_path, ref_path, output_dir,
    ...
    if 'CURRENT STATUS' in df.columns:
        ...
        df['STATUS_CODE'] = df['CURRENT STATUS'].str.extract(r'^(-?\d+)')[0]
    else:
        df['STATUS_CODE'] = ''
    ...
    if 'STATUS_CODE' in dm.columns:
        dm = dm[~dm['STATUS_CODE'].isin(['410', '201', '520', '-99', '99', '100'])].copy()
    ...
    for handle in unique_handles:
        ...
        for rn in ['Pickup', 'Delivery', 'Pending']:
            ...
            if rn == 'Pending':
                ...
                if not df_h.empty:
                    ...
                    df_h_transit = pd.DataFrame()
                    df_h_action = pd.DataFrame()
                
                # Transit section
                rows, total, active_days = build_section_rows(df_h_transit, icols, day_cols, date_col)
                sections.append(('កំពុងលើផ្លូវ (Transit)', rows, total, icols, active_days))
                counts['Pending_Transit'] = total
                    
                # Action section
                rows, total, active_days = build_section_rows(df_h_action, icols, day_cols, date_col)
                sections.append(('ដល់ប៉ុស្តិ៍ (Action Needed)', rows, total, icols, active_days))
                counts['Pending_Action'] = total
                    
                counts['Pending'] = counts['Pending_Transit'] + counts['Pending_Action']
                overall['Pending'] += counts['Pending']
            else:
        ...
        handle_files = []
        if sections:
            # Use current datetime timestamp instead of date (to avoid _000000 time)
            timestamp_str = datetime.now().strftime('%d_%m_%Y_%H%M%S')
            tmp_xlsx = os.path.join(output_dir, f"Report_{handle}_{timestamp_str}.xlsx")
            build_handle_excel(handle, sections, day_cols, excel_design, tmp_xlsx, mode=mode, order_created_map=order_created_map, order_status_map=order_status_map)
